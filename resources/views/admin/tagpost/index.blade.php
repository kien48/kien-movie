...
@section('content')
    ...
    <div class="d-flex justify-content-between mt-3 mb-3">
        <h1 class="text-center h3">Danh sách tag bài viết</h1>
        <a href="{{route('admin.tagposts.create')}}" class="btn btn-primary">Thêm mới</a>
    </div>
    @if(session('error'))
        ...
        @foreach($data as $tag)
            <tr>
                ...
                <td class="text-nowrap" style="width: 1px;">
                    <a href="{{route('admin.tagposts.edit', $tag->id)}}" class="btn btn-outline-warning">Sửa</a>
                    <form action="{{route('admin.tagposts.destroy', $tag->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
